...
function OutputLoggly() {
  ...
  this.config = {
    ...
    required_params: ['customer_token'],
    optional_params: ['error_buffer_delay', 'proto', 'send_json', 'loggly_tags'],
    default_values: {
...
OutputLoggly.prototype.afterLoadConfig = function(callback) {
  ...
  this.http_options = {
    ...
    path: '/inputs/'+this.customer_token+'/',
    method: 'POST',
	headers: {
		'X-LOGGLY-TAG': this.loggly_tags,
		'Content-Type': (this.send_json ? 'application/x-www-form-urlencoded' : 'text/plain')
	}
  };
  callback();
}
